...
export default function Home() {
    return <main>
        ...
        <section className="border-b-[3px] border-ink py-12 sm:py-16"><div className="mx-auto grid max-w-7xl items-center gap-10 px-5 sm:px-8 lg:grid-cols-[1.05fr_.95fr]">
            <Reveal><div><div className="mb-5 inline-flex items-center gap-2 rounded-full border-[3px] border-ink px-3 py-2 text-xs font-extrabold"><span className="text-base">✦</span>PENDAFTARAN DIBUKA</div><h1 className="font-display text-5xl leading-[.98] sm:text-7xl">Wujudkan Masa Depanmu Mulai dari SMK Bani Masum!</h1><p className="mt-5 max-w-xl text-lg leading-8">Belajar keahlian yang benar-benar dipakai di dunia kerja, dari praktik nyata sampai bimbingan guru yang siap mendampingi kamu sampai lulus.</p><div className="mt-7 flex flex-wrap gap-4"><Link href="/ppdb/daftar" className="comic-button border-[3px] border-ink bg-ink px-6 py-3 font-bold text-paper comic-shadow">Daftar Sekarang</Link><a href="#jurusan" className="comic-button border-[3px] border-ink bg-paper px-6 py-3 font-bold comic-shadow">Lihat Jurusan</a></div></div></Reveal>
            <Float><StudentArt /></Float>
        </div></section>

        <section id="keunggulan" className="border-b-[3px] border-ink py-16"><div className="mx-auto max-w-7xl px-5 sm:px-8"><Reveal><p className="mb-2 text-xs font-extrabold uppercase tracking-wide">Kenapa SMK Bani Masum</p><h2 className="font-display text-4xl sm:text-5xl">Bukan Cuma Sekolah, Tapi Bekal Kerja</h2><p className="mt-3 max-w-2xl leading-7 text-neutral-700">Empat hal yang bikin lulusan kami siap terjun ke dunia kerja maupun lanjut kuliah.</p></Reveal><StaggerGroup className="mt-9 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{advantages.map(([number, title, text]) => <StaggerItem key={title}><article className="border-[3px] border-ink bg-paper p-5 comic-shadow"><span className="font-display text-4xl">{number}</span><h3 className="mt-4 text-lg font-extrabold">{title}</h3><p className="mt-2 text-sm leading-6 text-neutral-700">{text}</p></article></StaggerItem>)}</StaggerGroup></div></section>

        <section id="jurusan" className="border-b-[3px] border-ink bg-paper-soft py-16"><div className="mx-auto max-w-7xl px-5 sm:px-8"><Reveal><p className="mb-2 text-xs font-extrabold uppercase tracking-wide">Kompetensi Keahlian</p><h2 className="font-display text-4xl sm:text-5xl">Pilih Jurusan Sesuai Minatmu</h2></Reveal><StaggerGroup className="mt-9 grid gap-5 md:grid-cols-2">{majors.map(([code, name, text]) => <StaggerItem key={code}><article className="overflow-hidden border-[3px] border-ink bg-paper comic-shadow"><div className="border-b-[3px] border-ink p-6"><span className="font-display text-5xl">{code}</span></div><div className="p-5"><h3 className="font-extrabold">{name}</h3><p className="mt-2 text-sm leading-6 text-neutral-700">{text}</p></div></article></StaggerItem>)}</StaggerGroup></div></section>
        ...
        <section id="jadwal" className="border-b-[3px] border-ink py-16"><div className="mx-auto max-w-7xl px-5 sm:px-8"><Reveal><p className="mb-2 text-xs font-extrabold uppercase tracking-wide">Jadwal PPDB</p><h2 className="font-display text-4xl sm:text-5xl">Jangan Sampai Kelewat Tanggal</h2></Reveal><Reveal delay={0.08}><div className="mt-9 overflow-x-auto border-[3px] border-ink comic-shadow"><table className="w-full min-w-[620px] border-collapse text-left text-sm"><thead className="bg-ink text-paper"><tr><th className="p-4 font-display text-lg">Tahapan</th><th className="p-4 font-display text-lg">Gelombang 1</th><th className="p-4 font-display text-lg">Gelombang 2</th></tr></thead><tbody>{[["Pendaftaran online", "2 Feb – 28 Feb", "3 Mar – 30 Apr"], ["Verifikasi berkas", "1 Mar – 5 Mar", "1 Mei – 5 Mei"], ["Pengumuman", "10 Mar", "10 Mei"], ["Daftar ulang", "11 – 15 Mar", "11 – 15 Mei"]].map((row) => <tr key={row[0]} className="border-b-2 border-ink last:border-0"><td className="p-4">{row[0]}</td><td className="p-4 font-bold">{row[1]}</td><td className="p-4 font-bold">{row[2]}</td></tr>)}</tbody></table></div></Reveal></div></section>
        ...
        <section className="py-16"><div className="mx-auto max-w-7xl px-5 sm:px-8"><Reveal><div className="border-[3px] border-ink bg-ink px-6 py-12 text-center text-paper comic-shadow-lg sm:px-10"><h2 className="font-display text-4xl sm:text-5xl">Siap Gabung Angkatan Baru?</h2><p className="mx-auto mt-3 max-w-xl text-paper/80">Isi formulir pendaftaran online sekarang, cuma butuh 10 menit.</p><Link href="/ppdb/daftar" className="comic-button mt-7 inline-flex border-[3px] border-ink bg-paper px-6 py-3 font-bold text-ink shadow-[5px_5px_0_#F3F2EE]">Mulai Daftar</Link></div></Reveal></div></section>
